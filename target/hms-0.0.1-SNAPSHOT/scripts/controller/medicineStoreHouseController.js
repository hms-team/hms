 ...
 */
var medicineAmtModule = angular.module("medicineStoreHouseModule",[]);
medicineAmtModule.controller("medicineStoreHouseController",function ($scope, $http) {
    ...
    $scope.consume = function (medAmt,storeAmt) {
        console.log("preparing consume "+storeAmt+" drug with id = "+medAmt.medId+" at "+ new Date());
        if(isNaN(storeAmt)){
            alert("请输入正确的数量");
            return;
        }
        var temp = parseInt(medAmt.amountStorehouse) - parseInt(storeAmt);
        if (temp < 0){
            console.log(medAmt.medId+" is not enough");
            alert("药房库存不足");
            return;
        }
        medAmt.amountStorehouse = temp;
        $http({
            method:"POST",
            url:"/hms/medicineAmt/transfer",
            data:{medId:medAmt.medId,source:"STORE_HS",destination:"CONSUME",amount:storeAmt,date:new Date()}
        }).success(function (data, status, headers, config) {
            console.log("success:"+data);
            var jsonResult = angular.fromJson(data);
